                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.payment-statuses.update", [$paymentStatus->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.paymentStatus.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $paymentStatus->name) }}" required>
                            @if($errors->has('name'))
